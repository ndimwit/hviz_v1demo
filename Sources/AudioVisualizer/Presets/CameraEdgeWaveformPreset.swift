...
/// Camera Edge Waveform visualizer preset
/// Performs edge detection on camera frames and applies waveform-based displacement
public struct CameraEdgeWaveformPreset: VisualizerPreset {
    public let id = "camera_edge_waveform"
    public let displayName = "Camera Edge (Waveform)"
...
/// Metal view for camera edge waveform visualization
private struct CameraEdgeWaveformMetalView: UIViewRepresentable {
    ...
    class Coordinator: NSObject, MTKViewDelegate {
        ...
        func setupMetal(device: MTLDevice, view: MTKView) {
            ...
            print("✓ Camera Edge Metal library loaded successfully")
            ...
            guard let vertexFunc = finalLibrary.makeFunction(name: "cameraEdgeVertex"),
                  let fragmentFunc = finalLibrary.makeFunction(name: "cameraEdgeWaveformFragment") else {
            ...
            do {
                ...
                print("✓ Camera Edge render pipeline created successfully")
            } catch {
                print("ERROR: Failed to create Camera Edge render pipeline: \(error)")
                return
            }
        ...
        func updateData(
            ...
            self.maxAmplitude = rawAudioSamples.map { abs($0) }.max() ?? 1.0
        ...
        func draw(in view: MTKView) {
            ...
            renderEncoder.setFragmentBytes(&opacityValue, length: MemoryLayout<Float>.stride, index: 5)
